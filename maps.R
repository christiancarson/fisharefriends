...
f <- round(h / 14)
svg <- c(sprintf('<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 %d" preserveAspectRatio="xMinYMin meet" fill="none" stroke="#111" stroke-linecap="round" stroke-linejoin="round">', h + f + 50), sprintf('<path d="%s" stroke-width="%d"/>', d, thick), sprintf('<text x="20" y="%d" font-size="%d" fill="#111" stroke="none">%s</text>', h + f + 30, f, tolower(river)), "</svg>")
