...
new post: `hugo new content posts/<slug>/index.md`, set `categories` to one or more of spey fishing, single hand, music, floats, fly tying, film, art, write under the front matter. Photos sit beside index.md and go in as `![](photo.jpg)`. Music: `{{< youtube ID >}}` or paste an embed iframe.

new trip: `hugo new content trips/<slug>/index.md`, set `kind` to raft (three spots) or wade (four spots), fill in trip_date, where (the place, with the river in brackets), river (names the map file), meet, crew, donate and donate_url (the group that gets the donations), bring. Every card links the BC freshwater licence page from `licence` in hugo.toml, and shows the river map `assets/maps/<river>.svg` when one exists: `Rscript maps.R "Squamish River"` draws it from the BC Freshwater Atlas (needs R with sf). Keep `crew: []` until someone signs up. Friends fill in the sign-up form, which opens an email with their name, whether they need gear, and their wader size; add their name to crew.
