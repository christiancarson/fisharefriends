...
new post: `hugo new content posts/<slug>/index.md`, set `categories` to one or more of spey fishing, single hand, music, floats, fly tying, film, art, write under the front matter. Photos sit beside index.md, landscape, and go in as `![caption](photo.jpg)`: they show 484 wide with the caption under, and open full size on click. Add `river: "Morice River"` to the front matter and the post shows that river map (see maps.R). Music: `{{< youtube ID >}}` or paste an embed iframe.
