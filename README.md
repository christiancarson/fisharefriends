...
new trip: `hugo new content trips/<slug>/index.md`, set `kind` to raft (three spots) or wade (four spots), fill in trip_date, where, meet, crew, bring. Every card links the BC freshwater licence page from `licence` in hugo.toml. Keep `crew: []` until someone signs up. Friends fill in the sign-up form, which opens an email with their name, whether they need gear, and their wader and boot sizes; add their name to crew.
