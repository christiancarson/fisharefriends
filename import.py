import pathlib, re, datetime, subprocess, plistlib, tomllib
...
waters = tomllib.loads(pathlib.Path("hugo.toml").read_text())["params"].get("waters", {})
...
for folder in sorted(p for p in src.iterdir() if p.is_dir()):
    ...
    for f in sorted(p for p in folder.iterdir() if not p.name.startswith(".")):
        ...
        if f.suffix.lower() in (".jpg", ".jpeg", ".png", ".heic"):
            tag = [waters.get(t, t) for t in finder_tags(f) or ([clean(t) for t in bits[1].split(",")] if len(bits) > 1 else ["fishies"])]
            tags.update(tag)
